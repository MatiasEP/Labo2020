...
<?php
	...
	if($id != '')
    {
        ...
        $rows = $client->executeQuery("proyecto.seguidos", $query); // $mongo contains the connection object to MongoDB
        $array = array();
        foreach($rows as $r)
        {
            foreach($r->seguidos as $idseguidos)
            {
                $filter2 = ["_id"=>$idseguidos];
                $options2 = ['sort' =>['_id'=>-1],];
                $query2 = new MongoDB\Driver\Query($filter2,$options2);
                $rows2 = $client->executeQuery("proyecto.usuarios", $query2);   
